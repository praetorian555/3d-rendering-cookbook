...
void Run() {
    ...
    const Opal::StringUtf8 vertex_shader_path = Opal::Paths::Combine(nullptr, "shaders", "cube-map-duck-vert.glsl").
            GetValue();
    const Opal::StringUtf8 frag_shader_path = Opal::Paths::Combine(nullptr, "shaders", "cube-map-duck-frag.glsl").
            GetValue();
    ...
    const Rndr::InputLayoutDesc input_layout_desc =
            builder.AddVertexBuffer(model_vertex_buffer, 1, Rndr::DataRepetition::PerVertex).AddIndexBuffer(
                model_index_buffer).Build();
    ...
    const Opal::StringUtf8 model_texture_path = Opal::Paths::Combine(nullptr, assets_root, "duck-base-color.png").
            GetValue();
    ...
    const Opal::StringUtf8 equirectangular_path = Opal::Paths::Combine(nullptr, assets_root, "piazza_bologni_1k.hdr").
            GetValue();
    const Rndr::Bitmap equirectangular_image = Rndr::File::ReadEntireImage(
        equirectangular_path, Rndr::PixelFormat::R32G32B32_FLOAT);
    ...
    const Opal::ArrayView<const u8> cube_map_data{
        cube_map_bitmap.GetData(), static_cast<size_t>(cube_map_bitmap.GetSize3D())
    };
    const Rndr::Texture cube_map_image{
        graphics_context,
        {
    ...
    const Opal::StringUtf8 cube_map_vertex_shader_path = Opal::Paths::Combine(nullptr, "shaders", "cube-map-vert.glsl").
            GetValue();
    const Opal::StringUtf8 cube_map_vertex_shader_code = Rndr::File::ReadShader(
        assets_root, cube_map_vertex_shader_path);
    const Opal::StringUtf8 cube_map_pixel_shader_path = Opal::Paths::Combine(nullptr, "shaders", "cube-map-frag.glsl").
            GetValue();
    const Opal::StringUtf8 cube_map_pixel_shader_code = Rndr::File::ReadShader(assets_root, cube_map_pixel_shader_path);
    Rndr::Shader cube_map_vertex_shader{
        graphics_context, {.type = Rndr::ShaderType::Vertex, .source = cube_map_vertex_shader_code}
    };
    Rndr::Shader cube_map_pixel_shader{
        graphics_context, {.type = Rndr::ShaderType::Fragment, .source = cube_map_pixel_shader_code}
    };
    ...
    const Rndr::Pipeline cube_map_pipeline{
        graphics_context,
        {
    ...
    while (!window.IsClosed()) {
        ...
        // Draw cube map.
        const Rndr::Matrix4x4f cube_map_model = Opal::Scale(2.0f);
        const Rndr::Matrix4x4f cube_map_mvp = Opal::Transpose(p * cube_map_model);
...
bool LoadMesh(const Opal::StringUtf8 &file_path, Opal::DynamicArray<VertexData> &vertices,
    ...
    for (unsigned i = 0; i != mesh->mNumVertices; i++) {
        ...
        vertices.PushBack({
            .pos = Rndr::Point3f(v.x, v.y, v.z), .normal = Rndr::Normal3f(n.x, n.y, n.z), .tc = Rndr::Point2f(t.x, t.y)
        });
    }
...
Rndr::Bitmap ConvertEquirectangularMapToVerticalCross(const Rndr::Bitmap &input_bitmap) {
    ...
    const Opal::InPlaceArray<Vector2i, k_face_count> k_face_offsets = {
        Vector2i(2 * face_size, face_size), Vector2i(0, face_size), Vector2i(face_size, 0),
    ...
    for (int face = 0; face < k_face_count; face++) {
        for (int x_result = 0; x_result < face_size; x_result++) {
            for (int y_result = 0; y_result < face_size; y_result++) {
                ...
                const float plane_distance = Opal::Sqrt(point.x * point.x + point.y * point.y);
                ...
                const float uf = 2.0f * face_size_float * (theta + Opal::k_pi_float) / Opal::k_pi_float;
                const float vf = 2.0f * face_size_float * (Opal::k_pi_float / 2.0f - phi) / Opal::k_pi_float;
                // 4-samples for bilinear interpolation
                const int32_t u1 = std::clamp(static_cast<int32_t>(Opal::Floor(uf)), 0, clamp_w);
                const int32_t v1 = std::clamp(static_cast<int32_t>(Opal::Floor(vf)), 0, clamp_h);
                ...
                const Rndr::Vector4f color = a * (1 - s) * (1 - t) + b * (s) * (1 - t) + c * (1 - s) * t + d * (s) * (
                                                 t);
                result.SetPixel(x_result + k_face_offsets[face].x, y_result + k_face_offsets[face].y, 0, color);
            }
        };
    }
